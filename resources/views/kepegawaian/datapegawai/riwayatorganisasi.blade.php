...
<nav class="page-breadcrumb">
    <ol class="breadcrumb breadcrumb-arrwo">
        ...
        <li class="breadcrumb-item active" aria-current="page">Data Riwayat Organisasi Pegawai</li>
    </ol>
</nav>
                    ...
                    <div class="card-header">
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center">
                                <div class="ms-2">
                                    <h6 class="card-title mb-0"><i class="mdi mdi-google-circles-extended icon-lg mx-2"></i> Data Riwayat Organisasi Pegawai</h6>     
                                </div>
                            </div>
                            <div class="pull-right">
                                <a class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#createOrganisasi">Tambah Riwayat Organisasi Baru</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        @forelse ($riwayat_organisasis as $key => $organisasi)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <form onsubmit="return confirm('Apakah Anda Yakin mengahapus data organisasi {{$organisasi->nama}}?');" action="{{ route('riwayat_organisasi.destroy', $organisasi->id) }}" method="POST">
                                                    <button type="button" class="btn btn-warning btn-xs btn-icon" data-id="{{ $organisasi->id }}"  data-bs-toggle="modal" data-bs-target="#editOrganisasi">
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="6"><small>Tidak ada data Riwayat Organisasi Pegawai</small></td>
                                        </tr>
                                        @endforelse
                                    </tbody>
                                </table>
...
<div class="modal fade" id="createOrganisasi" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-focus="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Form Tambah Riwayat Organisasi Baru</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
            </div>
            <form action="{{route('riwayat_organisasi.store')}}" method="post">
                @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-sm-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="nama" id="" placeholder="Nama Organisasi">
                                    @error('nama')
                                        <code>{{$message}}</code>
                                    @enderror
                                </div>
                            ...
                            <div class="col-lg-6 col-sm-12">
                                <div class="mb-3">
                                    ...
                                    <textarea name="keterangan" class="form-control" id="" cols="30" rows="5" placeholder=""></textarea>
                                    @error('keterangan')
                                        <code>{{$message}}</code>
                                    @enderror
                                </div>
...
<div class="modal fade" id="editOrganisasi" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-focus="false">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Form Ubah Riwayat Organisasi</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
            </div>
            <form id="editOrganisasiForm" method="post">
                @csrf
                @method('PUT')
                ...
                <div class="modal-body">
                    <div class="container"><div class="row">
                        ...
                        <div class="col-lg-6 col-sm-12">
                            <div class="mb-3">
                                <label for="" class="form-label">Keterangan</label>
                                <textarea name="keterangan" class="form-control" id="keterangan" cols="30" rows="5" placeholder=""></textarea>
                                @error('keterangan')
                                    <code>{{$message}}</code>
                                @enderror
                            </div>
...
<script>
    $(document).ready(function() {
        var baseUrl = '{{ url('/') }}';
        $('#editOrganisasi').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget);
            var organisasiId = button.data('id');
            var url = baseUrl + '/kepegawaian/riwayat_organisasi/edit/' + organisasiId;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var formAction = "{{ route('riwayat_organisasi.update', ':id') }}";
                    formAction = formAction.replace(':id', data.id);
                    $('#editOrganisasiForm').attr('action', formAction);
                    },
                error: function() {
